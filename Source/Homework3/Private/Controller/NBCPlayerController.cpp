...
void ANBCPlayerController::ShowMainMenu(bool bIsRestart)
{
	...
	if (MainMenuWidgetClass)
	{
		...
		if (bIsRestart)
		{
			...
			{
				if (UHomeworkGameInstance* HomeworkGameInstance = Cast<UHomeworkGameInstance>(UGameplayStatics::GetGameInstance(this)))
				{
					TotalScoreText->SetText(FText::FromString(FString::Printf(TEXT("Total Score : %d"), HomeworkGameInstance->TotalScore)));
				}
			}
		}
	}
...
void ANBCPlayerController::StartGame()
{
	if (UHomeworkGameInstance* HomeworkGameInstance = Cast<UHomeworkGameInstance>(UGameplayStatics::GetGameInstance(this)))
	{
		HomeworkGameInstance->CurrentLevelIndex = 0;
		HomeworkGameInstance->TotalScore = 0;
	}

	UGameplayStatics::OpenLevel(GetWorld(), FName("BasicLevel"));
}

void ANBCPlayerController::QuitGame()
{
